...
app.get('/receipt/donationInfo', (req, res) => {
    connection.query("select A.donation_id, C.affiliation, C.username, B.column_type, B.detail from donations A, donation_column B, users C where A.donation_id=B.donation_id and B.column_type in ('Name', 'Price', 'Quantity') and A.is_new=? and A.company_id = C.username", [true],(err, result) => {
        if(err) throw err;
